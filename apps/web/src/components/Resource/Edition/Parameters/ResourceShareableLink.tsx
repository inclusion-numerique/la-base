...
const ResourceShareableLink = ({ resource }: { resource: Resource }) => {
  const [shareLink, setShareLink] = useState(resource.shareableLink)

  const form = useForm<UpdateShareableLinkCommand>({
    defaultValues: {
      enabled: !!resource.shareableLink?.enabled,
  ...
  const shareUrl = shareLink?.enabled
    ? getServerUrl(`/ressources/${shareLink.id}`, { absolutePath: true })
    : null
  ...
  }, [enabledValue, shareLink, resource])
      ...
      <Card
        ...
        title={
          <h2 className="fr-mb-3v fr-h5 fr-text-label--blue-france">
            Partage de la ressource via un lien
          </h2>
        }
        titleAs="div"
        contentSeparator
...
export default withTrpc(ResourceShareableLink)
