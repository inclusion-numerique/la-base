...
import type { ResourceRepublished } from './RepublishResource'
import { onRepublished } from './RepublishResource.server'

describe('onRepublished', () => {
  ...
  const mockEvent: ResourceRepublished = {
    type: 'Republished',
    timestamp: new Date(),
    data: {
      __version: 1,
  ...
  it('should call the resource repository to update the resource with contents containing order', async () => {
    ...
    await onRepublished(mockEvent, resource, {
